...
export const dateUtils = Object.freeze({
    ...
    locationOfTheDateCompareToOtherDate(
        ...
        const isTheDateOnTheFuture =
            secondDateWithoutHOur > firstDateWithoutHour;
        const isTheDatesAreExactSame = this.isTheDatesAreEqual(
            firstDateWithoutHour,
            secondDateWithoutHOur,
        );

        const isTheDatesAreDifferentButSameYear =
            isTheYearSame && !isTheSameMonth;
        const isTheDatesAreDifferentButSameMonth =
            !isTheYearSame && isTheSameMonth;

        const isTheDateOnThePastButSameMonth =
            isTheDateOnThePast && isTheSameMonth;
        const isTheDateOnTheFutureButSameMonth =
            isTheDateOnTheFuture && isTheSameMonth;
        const isTheDateOnTheSameYearAndSameMonth =
            isTheYearSame && isTheSameMonth;
